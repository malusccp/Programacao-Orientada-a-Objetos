...
class TestesBancoLimite {
    ...
    private _pontosObtidos: number = 0;
    private _pontosTotais: number = 0;
    ...
    private rodarTeste(nome: string, pontos: number, fn: () => void): void {
    ...
    private iniciarBanco(): void {
        this._banco = new Banco();
    }

    private criarClienteNoBanco(nome: string, cpf: string): Cliente {
        const cliente = new Cliente(nome, cpf, new Date(1990, 0, 1));
        this._banco.inserirCliente(cliente);
        return cliente;
    }

    private criarContaNoBanco(numero: string, saldo: number, limite: number): Conta {
        const conta = new Conta(numero, saldo, limite);
    ...
    private teste1Deposito(): void {
        this.rodarTeste(
            "Teste 1: Depósito registra saldo e operação corretamente",
            1.0,
            () => {
                const conta = new Conta("001", 0, 0);
    ...
    private teste2SaqueComLimite(): void {
        this.rodarTeste(
            "Teste 2: Saques respeitam limite e registram falha",
            1.5,
            () => {
                const conta = new Conta("002", 100, 100); // saldo 100, limite 100
                ...
                this.assert(
                    valores[0] == 50 && valores[1] == 100 && valores[2] == 100,
                    "Os valores das operações deveriam ser 50, 100, 100."
                );
            }
        );
    }
    ...
    private teste3TransferenciaComLimite(): void {
        this.rodarTeste(
            "Teste 3: Transferências respeitam limite e retornam operações corretas",
            2.0,
            () => {
                const origem = new Conta("100", 100, 200);
                const destino = new Conta("200", 50, 0);
    ...
    private teste4BancoRegistroGeral(): void {
        this.rodarTeste(
            "Teste 4: Banco registra corretamente operações no extrato geral",
            2.5,
            () => {
                this.iniciarBanco();
                ...
                this.assert(
                    extratoConta1.length == 3,
                    "Conta 301 deveria ter 3 operações (depósito, saque e falha de transferência)."
                );
                ...
                this.assert(
                    conta1.saldo == 50,
                    "Saldo final da conta 301 deveria ser 50."
                );

                this.assert(
                    extratoGeral.length >= extratoConta1.length,
                    "Extrato geral deve ter pelo menos as operações da conta 301."
                );
                ...
                for (let i = 0; i < idsConta1.length; i++) {
                    this.assert(
                        idsGeral.indexOf(idsConta1[i]) != -1,
                        "Operação da conta 301 não encontrada no extrato geral do banco."
                    );
                }
            }
        );
    ...
    private teste5ClienteTresContas(): void {
        this.rodarTeste(
            "Teste 5: Operações via Banco para cliente com 3 contas e consistência de extratos",
            3.0,
            () => {
                this.iniciarBanco();
                const banco = this._banco;
                ...
                const uniaoContas = ([] as Operacao[])
                    .concat(extrato401)
                    .concat(extrato402)
                    .concat(extrato403);

                const idsCliente = extratoCliente.map(o => o.id).sort((a, b) => a - b);
                const idsUniao = uniaoContas.map(o => o.id).sort((a, b) => a - b);

                this.assert(
                    idsCliente.length == idsUniao.length,
                    "Quantidade de operações no extrato do cliente difere da soma das 3 contas."
                );

                for (let i = 0; i < idsCliente.length; i++) {
                    this.assert(
                        idsCliente[i] == idsUniao[i],
                        "Conjunto de operações do extrato do cliente não corresponde às operações das 3 contas."
                    );
                }
                ...
                for (let i = 0; i < idsCliente.length; i++) {
                    this.assert(
                        idsGeral.indexOf(idsCliente[i]) != -1,
                        "Operação do extrato do cliente não encontrada no extrato geral do banco."
                    );
                }
            }
        );
