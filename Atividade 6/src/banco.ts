export class Conta {
    ...
    constructor(id: number, cliente: Cliente | undefined, dataAbertura: Date, numero: string, saldo: number) {
        this.id = id;
...
export class Banco {
    ...
    transferirParaMuitas(numeroOrigem: string, numerosContasDestino: string[], valorPorTransferencia: number): void {
        let contaOrigem = this.consultarPorNumero(numeroOrigem);
        if (!contaOrigem) {
    ...
    obterQuantidadeContas(): number {
        return this.contas.length;
    }

    obterTotalDepositado(): number {
    ...
    mudarTitularidade(numeroConta: string, cpfNovoCliente: string): void {
