...
class Produto{
    ...
    aplicarDesconto(percentual: number){
        let desconto: number = this.preco * (percentual/100);
        return this.preco - desconto


    }
    emitirOrcamento(percentualDeDesconto: number){
        let novoPreco: number = this.aplicarDesconto(percentualDeDesconto);
